...
import { uploadToS3, deleteFromS3, type UploadProgress } from '../utils/s3Upload';
import { uploadToBackend } from '../utils/backendApi';
...
export function MediaUploader({
  ...
  const processFiles = async (newFiles: File[]) => {
    console.log('⚙️ Processing files:', newFiles.length);
    setProcessing(true);
  ...
  const removeFile = async (id: string) => {
    console.log('🗑️  removeFile called for id:', id);
    const fileToDelete = files.find(f => f.id === id);
    console.log('   File to delete:', fileToDelete);
    
    // If file is uploaded to S3, delete it from S3 first
    if (fileToDelete?.s3Key && user) {
      console.log('   File has s3Key, deleting from S3:', fileToDelete.s3Key);
      try {
        // Show loading state
        ...
        const result = await deleteFromS3(fileToDelete.s3Key);
        console.log('   Delete result:', result);
        
        if (!result.success) {
          console.error('Failed to delete file from S3:', result.error);
  ...
  const uploadFiles = async () => {
    ...
    setTimeout(() => {
      setFiles(currentFiles => {
        const uploadedFiles = currentFiles.filter(f => f.uploaded);
        console.log('📤 Calling onUploadComplete with:', uploadedFiles.length, 'files');
        uploadedFiles.forEach(f => {
          console.log('  File:', f.file.name, 'S3 URL:', f.s3Url);
        });
        onUploadComplete(uploadedFiles);
        return []; // Clear files after callback
      });
    }, 500);
  };
                      ...
                      <div className="mt-2">
                        <div className="flex items-center justify-between text-sm text-[var(--sunset-orange)] font-serif mb-1">
                          <span className="flex items-center gap-2">
                            ...
                            {useS3Upload ? 'Uploading to S3...' : 'Uploading...'}
                          </span>
                    ...
                    {file.uploaded && (
                      <div className="flex items-center gap-2 mt-2">
                        <div className="flex items-center gap-2 text-green-600 text-sm font-serif">
                          ...
                          {file.s3Url ? 'Uploaded to S3' : 'Uploaded'}
                        </div>
                        {file.s3Url && (
                          <a
                            href={file.s3Url}
